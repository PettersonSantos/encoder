...
func (vu *VideoUpload) ProcessUpload(concurrency int, doneUpload chan string) error {
	...
	returnChannel := make(chan string) //avisa quando cada upload terminar, pode ser um erro ou ok
	...
	for process := 0; process < concurrency; process++ {
		go vu.uploadWorker(in, returnChannel, uploadClient, ctx) //cria uma goroutine;
	}

	go func() { //função anonima
		for x:=0; x < len(vu.Paths); x++ {
			in <- x //passando pra variavel in o valor de x; espera o in ser esvaziado pra atribuir o valor de x;
		}
		close(in)
	}()
